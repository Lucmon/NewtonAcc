...
def pfb(qp,z, _lambda,v, sigma,inner_tol, alpha, niters, max_iters, inverse_time, c_feature,x_iterate_gt,CHNet, optimizer, scheduler):
    ...
    for j in range(max_inner_iters):
        ...
        if niters >= 1:
            ...
            # mse loss + l1 loss
            #loss = mseLoss + 0.52 * l1Loss + nRLoss
            if cfg._use_gt_loss:
            ...
            """
            for p in CHNet.named_parameters():
                #print(p)
                print(p[0])
                print(p[1].grad)
                print(p[1])
        ...
        #print(mu.shape)
        #print(mu_true.shape)
        #print(torch.sum(torch.abs(gamma-gamma_true)))
        #print(torch.sum(torch.abs(mu-mu_true)))
        #new iteration
        if cfg.data_type == "simulation":
            m, n = qp.G.shape
        elif cfg.data_type == "svm":
        ...
        if cfg.data_type == "simulation":
            ...
            #R = torch.cat([r1,r3], dim = 0)
            #print("===  DIAG: ===")
            #print(torch.sum(torch.abs(torch.matmul(K_inv, K)), dim = 1))
            #print(torch.diag(torch.matmul(K_inv, K)))
    ...
    return z, _lambda, v, niters, inverse_time, nR, nR_list, obj_list,constraint_num_list,x_list,v_list
...
def solve_qp(qp,x0,lambda0,v0,inverse_time,c_feature,x_iterate_gt,CHNet,optimizer,scheduler,cfg):
    ...
    for j in range(max_prox_iters):
        ...
        if (E <= tol + E0*rtol) or torch.norm(dx) + torch.norm(dlambda) + torch.norm(dv) <= dtol:
            ...
            break
        xp, lambdap, vp, newton_iters, inverse_time, nR,nR_list,obj_list,constraint_num_list,x_list,v_list = pfb(qp, x, _lambda, v, sigma, inner_tol, alpha, \
        ...
        prox_iters += 1
    return x, inverse_time, nR_list,obj_list,constraint_num_list,x_list,v_list
...
def pfb_test(qp, z, _lambda, v, sigma, inner_tol, alpha, niters, max_iters, inverse_time,chnet_time, c_feature,x_iterate_gt, CHNet,protype):
    ...
    for j in range(max_inner_iters):
        ...
        if cfg.data_type == "simulation":
            ...
            """
            print("Calculating R...")
            v_argmax = torch.argmax(v.squeeze())
            print("V max:", v[v_argmax])
            constraint_argmax = qp.A[v_argmax]
            A_idx = np.where(constraint_argmax != 0)
            print("A IDX:", A_idx)
            dim_selected = qp.A.t()[A_idx[0][0]]
            dim_selected_not_zero = np.where(dim_selected != 0)
            print("dim selected not zero:", dim_selected[dim_selected_not_zero[0]])
            print("v with dim selected:", v[dim_selected_not_zero[0]].squeeze())
            print("Av:", torch.matmul(qp.A.t(), v)[A_idx[0]])
            print("r1:", r1[A_idx[0]])
            """
        ...
        if cfg.smw_acc:
            ...
            if cfg.data_type == "simulation":
                ...
                K = torch.cat([
                    torch.cat([qp.H, qp.A.t()], dim = 1),
                    torch.cat([-qp.A, torch.diag(mu/gamma)], dim=1)
                ])
                """
                #print("IN SVM K")
                K = torch.cat([
    ...
    if cfg.train:
        for epoch in range(10):
            for i in range(25):
                ...
                c_feature = qp.A
                x, _ , nR_list,obj_list_from_qp,constraint_num_list,x_list,v_list = solve_qp(qp, x0, lambda0, v0, inverse_time,c_feature,x_iterate_gt[i],CHNet,optimizer,scheduler,cfg)
